...
export default function PhysicalView({
  ...
  const handlePointerMove = (e) => { if (isDragging) updatePos(e); };
  ...
  const updatePos = (e) => {
    ...
    setProbePos(Math.max(10, Math.min(240, x * scale)));
  };
  ...
  const renderBeam = () => {
    ...
    if (probe.angle === 0) {
      ...
      if (block.id === "flaw" && flaws) {
        flaws.forEach((f) => {
          ...
          if (dx < f.size + 6 && f.y < hitDepth) hitDepth = f.y;
        });
      }
      return (
        <g>
          ...
          {hitDepth === block.thickness && (
            <path
              d={`M ${effectiveProbePos - 3},${hitDepth} L ${effectiveProbePos},${hitDepth - 10} L ${effectiveProbePos + 3},${hitDepth}`}
              fill={`rgba(${rgb}, 0.4)`}
            />
          )}
        </g>
    ...
    if (block.id === "k1") {
      ...
      if (distToCenter < 30 && probeDir === 1) {
        ...
        return <path d={`M ${effectiveProbePos},0 L ${nextX},${nextY}`} stroke={`rgba(${rgb}, 0.4)`} strokeWidth="12" fill="none" strokeLinecap="round" />;
      }
    } else if (block.id === "k2") {
      const distToCenter = Math.abs(effectiveProbePos - 25);
      if (distToCenter < 20) {
        const dist = probeDir === 1 ? 25 : 50;
        const nextX = effectiveProbePos + probeDir * dist * Math.sin(theta);
        const nextY = dist * Math.cos(theta);
        return <path d={`M ${effectiveProbePos},0 L ${nextX},${nextY}`} stroke={`rgba(${rgb}, 0.4)`} strokeWidth="12" fill="none" strokeLinecap="round" />;
      }
    }
  ...
  return (
    ...
    >
      <svg viewBox="-10 -30 270 100" className="w-full h-full overflow-visible">
        <defs>
          <pattern id="ut-grid" width="10" height="10" patternUnits="userSpaceOnUse">
            <path d="M 10 0 L 0 0 0 10" fill="none" stroke="rgba(255,255,255,0.05)" strokeWidth="0.5" />
          </pattern>
        </defs>
        <rect x="-10" y="-30" width="270" height="100" fill="url(#ut-grid)" pointerEvents="none" />

        {/* Rulers */}
        <g pointerEvents="none">
          {Array.from({ length: 26 }).map((_, i) => (
            <g key={`rx-${i}`} transform={`translate(${i * 10}, 0)`}>
              <line x1={0} y1={0} x2={0} y2={-2} stroke="#737373" strokeWidth="0.5" />
              {i % 5 === 0 && <text x={0} y={-3} fill="#737373" fontSize="4" textAnchor="middle">{i * 10}</text>}
            </g>
          ))}
          {Array.from({ length: Math.floor((block.thickness || 25) / 10) + 1 }).map((_, i) => (
            <g key={`ry-${i}`} transform={`translate(0, ${i * 10})`}>
              <line x1={0} y1={0} x2={-2} y2={0} stroke="#737373" strokeWidth="0.5" />
              {i % 2 === 0 && <text x={-3} y={1.5} fill="#737373" fontSize="4" textAnchor="end">{i * 10}</text>}
            </g>
          ))}
        </g>
              ...
              if (flawEcho) {
                ...
                measurementText = (
                  <g transform={`translate(${f.x}, ${f.y - f.size * 1.15 - 3})`} pointerEvents="none">
                    ...
                    <text x="0" y="-12" fill="#4ade80" fontSize="4" textAnchor="middle" fontFamily="monospace" fontWeight="bold">SA: {S.toFixed(1)}</text>
                    <text x="0" y="-6.5" fill="#a78bfa" fontSize="4" textAnchor="middle" fontFamily="monospace">PA: {PA.toFixed(1)} | RA: {RA.toFixed(1)}</text>
                  </g>
                );
              }
              return (
          ...
          {probe.angle > 0 && (
            <>
              <line
                ...
                stroke="rgba(255,255,255,0.5)" strokeWidth="2.3"
              />
              <line x1={probeDir * probeIndex} y1={0} x2={probeDir * probeIndex} y2={-5} stroke="#ef4444" strokeWidth="1.5" />
            </>
          )}
          <circle cx={0} cy={0} r={1.725} fill="white" />
          {probe.angle > 0 && <circle cx={probeDir * probeIndex} cy={0} r={1.5} fill="#ef4444" />}
          <text x={0} y={-25} fill="#9ca3af" fontSize="4" textAnchor="middle" fontFamily="monospace">{probePos.toFixed(1)} mm</text>
        </g>
      </svg>
    </div>
  );
