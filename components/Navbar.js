      ...
      <div className="flex items-center gap-6 text-sm">
        ...
        <div className="flex items-center bg-white/[0.05] rounded-full p-0.5 border border-white/10">
          <button onClick={() => switchLang("tr")}
            className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all border-none cursor-pointer font-sans ${lang === "tr" ? "bg-gold-400 text-dark-800" : "bg-transparent text-dark-300 hover:text-dark-50"}`}>
            TR
          </button>
          <button onClick={() => switchLang("en")}
            className={`px-2.5 py-1 rounded-full text-xs font-medium transition-all border-none cursor-pointer font-sans ${lang === "en" ? "bg-gold-400 text-dark-800" : "bg-transparent text-dark-300 hover:text-dark-50"}`}>
            EN
          </button>
        </div>
          ...
          {toolsOpen && (
            <div className="absolute top-10 left-0 bg-dark-800 border border-white/10 rounded-lg shadow-xl p-2 min-w-[220px] animate-fade-in">
              ...
              <Link href="/tools/unit-converter" onClick={() => setToolsOpen(false)}
                ...
                <span>📐</span>
                <div>
                  <div className="text-sm font-medium">{t.unitConverterShort}</div>
        ...
        <a href="#about" className="text-dark-100 hover:text-gold-400 transition-colors no-underline">{t.about}</a>

        {isSignedIn ? (
          <Link href="/dashboard" className="bg-gradient-to-br from-gold-400 to-gold-500 text-dark-800 no-underline rounded-md px-5 py-2 text-sm font-semibold hover:shadow-lg hover:shadow-gold-400/20 transition-all">
            {t.dashboard}
          </Link>
        ) : (
