...
const CATEGORIES = {
  length: {
    ...
    icon: "📏",
    units: {
      mm: { label: "Milimetre (mm)", factor: 1 },
      cm: { label: "Santimetre (cm)", factor: 10 },
      m: { label: "Metre (m)", factor: 1000 },
      um: { label: "Mikrometre (μm)", factor: 0.001 },
      inch: { label: "İnç (in)", factor: 25.4 },
      ft: { label: "Feet (ft)", factor: 304.8 },
      yd: { label: "Yard (yd)", factor: 914.4 },
    },
    base: "mm",
  },
  temperature: {
  ...
  pressure: {
    ...
    icon: "🔴",
    units: {
      mpa: { label: "Megapascal (MPa)", factor: 1 },
      psi: { label: "psi", factor: 0.00689476 },
      bar: { label: "Bar", factor: 0.1 },
      kgfcm2: { label: "kgf/cm²", factor: 0.0980665 },
      atm: { label: "Atmosfer (atm)", factor: 0.101325 },
      kpa: { label: "Kilopascal (kPa)", factor: 0.001 },
    },
    base: "mpa",
  },
  stress: {
    name: "Gerilme / Dayanım",
    icon: "💪",
    units: {
      mpa: { label: "Megapascal (MPa)", factor: 1 },
      ksi: { label: "ksi (kip/in²)", factor: 6.89476 },
      nmm2: { label: "N/mm²", factor: 1 },
      kgfmm2: { label: "kgf/mm²", factor: 9.80665 },
      gpa: { label: "Gigapascal (GPa)", factor: 1000 },
    },
    base: "mpa",
  },
  force: {
    ...
    units: {
      ...
      kgf: { label: "Kilogram-kuvvet (kgf)", factor: 9.80665 },
      lbf: { label: "Pound-kuvvet (lbf)", factor: 4.44822 },
      mn: { label: "Meganewton (MN)", factor: 1000000 },
    },
    base: "n",
  },
  energy: {
    ...
    units: {
      ...
      kgfm: { label: "kgf·m", factor: 9.80665 },
      kj: { label: "Kilojoule (kJ)", factor: 1000 },
      cal: { label: "Kalori (cal)", factor: 4.184 },
    },
    base: "j",
  },
  area: {
    ...
    icon: "⬜",
    units: {
      mm2: { label: "mm²", factor: 1 },
      cm2: { label: "cm²", factor: 100 },
      m2: { label: "m²", factor: 1000000 },
      in2: { label: "in²", factor: 645.16 },
      ft2: { label: "ft²", factor: 92903 },
    },
    base: "mm2",
  },
  mass: {
    ...
    units: {
      ...
      ton: { label: "Metrik Ton (t)", factor: 1000000 },
...
export default function UnitConverter() {
  ...
  const handleConvert = () => {
    ...
    for (const key of unitKeys) {
      if (cat.custom) {
        results[key] = convertTemperature(val, fromUnit, key);
      } else {
        results[key] = convertStandard(val, fromUnit, key, category);
      }
    }
        ...
        <div className="flex gap-2 mb-6 overflow-x-auto flex-wrap">
          {Object.entries(CATEGORIES).map(([key, c]) => (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-lg px-3 py-2 text-xs font-medium whitespace-nowrap transition-all cursor-pointer border font-sans ${
                category === key
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-6">
            ...
            <div className="mb-4">
              ...
              <select
                value={fromUnit}
                onChange={(e) => { setFromUnit(e.target.value); setAllResults(null); }}
              ...
              >
                {unitKeys.map((key) => (
                  <option key={key} value={key}>{cat.units[key].label}</option>
                ))}
              </select>
            </div>
            ...
            <div className="mb-6">
              ...
              <input
                ...
                className="w-full bg-dark-800 border border-white/10 rounded-lg px-3 py-2.5 text-lg text-dark-50 focus:border-gold-400/50 focus:outline-none font-mono"
              />
            </div>
          ...
          <div className="bg-white/[0.03] border border-white/[0.08] rounded-xl p-6">
            ...
            {allResults ? (
              <div className="space-y-2 animate-fade-in">
                {unitKeys.map((key) => {
                  ...
                  return (
                    ...
                    >
                      <div className="text-xs text-dark-300 max-w-[140px]">
                        {cat.units[key].label}
                      </div>
                      <div className={`text-lg font-bold font-mono ${
                        isSource ? "text-gold-400" : "text-dark-50"
                      }`}>
                        {formatNumber(allResults[key])}
                      </div>
                    </div>
                  );
                })}
              ...
              <div className="flex flex-col items-center justify-center h-64 text-dark-300">
                ...
                <div className="text-xs text-dark-300 mt-1">Tüm birimler aynı anda gösterilir</div>
              </div>
            )}
          </div>
        </div>
        ...
        <div className="mt-8 bg-white/[0.02] border border-white/[0.06] rounded-xl p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-dark-300">
            <div className="bg-dark-800 rounded-lg p-3">
              <div className="text-dark-100 font-medium mb-1">Gerilme</div>
              1 MPa = 0.145 ksi = 1 N/mm²
            </div>
            <div className="bg-dark-800 rounded-lg p-3">
              <div className="text-dark-100 font-medium mb-1">Darbe Enerjisi</div>
              1 J = 0.738 ft·lb = 0.102 kgf·m
            </div>
            <div className="bg-dark-800 rounded-lg p-3">
              <div className="text-dark-100 font-medium mb-1">Sıcaklık</div>
              °F = °C × 9/5 + 32 | K = °C + 273.15
            </div>
            <div className="bg-dark-800 rounded-lg p-3">
              <div className="text-dark-100 font-medium mb-1">Uzunluk</div>
              1 inch = 25.4 mm | 1 ft = 304.8 mm
            </div>
            <div className="bg-dark-800 rounded-lg p-3">
              <div className="text-dark-100 font-medium mb-1">Basınç</div>
              1 MPa = 145 psi = 10 bar
            </div>
            <div className="bg-dark-800 rounded-lg p-3">
              <div className="text-dark-100 font-medium mb-1">Kuvvet</div>
